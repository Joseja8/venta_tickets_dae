...
public class PriceTableImp implements PriceTable {
    ...
    public PriceTableImp() {
        this.prices = new ArrayList<>(new ArrayList<>(Arrays.asList(
                new PriceTableEntry(), new PriceTableEntry(), new PriceTableEntry())));
    }
    ...
    private PriceTableEntry getEntry(Event event, Zone zone) {
        for (PriceTableEntry entry : getPrices()) {
            if (entry.getEvent().equals(event) && entry.getZoneID().equals(zone.getType()))
                return entry;
        }
        return null;
    }
