...
public class EventManagerImp implements EventManager {
    ...
    @Override
    public List<Event> getEvents(LocalDate date, EventType type) {
        return eventDao.findByDateAndType(date, type);
    }

    @Override
    public List<Event> getEvents(LocalDate date, EventType type, String city) {
    ...
    @Override
    public boolean addPrice(Event event, Zone zone, float price, String token) {
        if (userManager.isAdmin(token)) {
            priceTable.addEntry(event, zone, price);
            return true;
        } else {
            return false;
        }
    ...
    @Override
    public Zone createZone(EventManager.ZoneType zone, int seats, String token) {
        if (userManager.isAdmin(token)) {
    ...
    @Override
    public EventManager.ZoneType[] getZoneTypes() {
        return EventManager.ZoneType.values();
    }

    @Override
    public EventManager.EventType[] getEventTypes() {
        return EventManager.EventType.values();
    }
